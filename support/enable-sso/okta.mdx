...
---
Single sign-on (SSO) lets your team members authenticate their access to the Mangopay Dashboard via a third-party identity provider.  
...
If you decide to set up SSO, Mangopay Dashboard users will no longer be able to authenticate using their previously created passwords.  
</Note>  
<Note>
**Note – Dashboard 2FA cannot be deactivated**

The two-factor authentication (2FA) of the Mangopay Dashboard cannot be deactivated under any circumstances.\
Okta allows you to deactivate 2FA for a particular app, such as the Dashboard, so you can simplify the experience and rely on Mangopay's feature, which will always remain in place.
</Note>
<Info>
**Prerequisites**
...
Contact the Mangopay Support team <a href="https://hub.mangopay.com" target="_blank">via the Dashboard</a>.
...
## 3. Await confirmation that the Okta Mangopay app is connected 
...
Now that the Okta Mangopay app has been enabled and connected, provision the users in Okta by assigning them to the app. This enables them to use Okta to connect to the Dashboard.
...
The sign-in process is initiated from Dashboard login page as before. The user needs to:

1. Navigate to the <a href="https://hub.mangopay.com" target="_blank">Dashboard login screen</a> in their browser.
2. Enter their Okta credentials and click "Sign in with Okta". The user is redirected to their Dashboard welcome screen.