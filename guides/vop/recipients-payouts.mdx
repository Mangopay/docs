...
The POST Create a Payout returns the VOP check performed previously on the Recipient, if it exists.

<Note>
**Note – Payout fails if Recipient not ACTIVE and User not verified**

If you don't complete SCA on Recipients created with the test data above, the payout request will fail immediately with the `ResultCode` [121006](/errors/codes/121006) – `The associated bank account is not active`, but the VOP check result will still be returned.
</Note> */}