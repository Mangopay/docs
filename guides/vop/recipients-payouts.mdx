...
In Sandbox, as in Production, active Bank Account objects have been migrated to Recipients, so you are able to call the [GET View a Recipient](/api-reference/recipients/view-recipient) with the `Id` of a legacy Bank Account. 

In Production, Mangopay will perform a new VOP check on the legacy Bank Account when you make the GET call, but in Sandbox, we won't. The same is true for Recipients created before the introduction of VOP.

This means that, in Sandbox, calling [GET View a Recipient](/api-reference/recipients/view-recipient) for a legacy Bank Account or Recipient created before VOP will always return:

```json Legacy Bank Account in Sandbox
    ...
    "RecipientVerificationOfPayee": {
...
The [POST Create a Payout](/api-reference/payouts/create-payout) returns the VOP check performed previously on the Recipient, if it exists.

In Production, Mangopay will perform an additional VOP check at the point of the payout initiation call and return it in the payout response. In Sandbox, however, a VOP check is only performed when the Recipient is created. Therefore, the payout response contains the same VOP check (same `RecipientVerificationId`) as the one returned on the Recipient.

The VOP result fields are not returned on the [GET View a Payout](/api-reference/payouts/view-payout) nor [GET View a Payout and check mode applied](/api-reference/payouts/view-payout-check-mode-applied) endpoints (in both Sandbox and Production).

<Note>
**Note – Payout fails if Recipient not ACTIVE and User not verified**

If you don't complete SCA on Recipients created with the test data above, then the Recipient `Status` is not `ACTIVE` and the payout will fail immediately. The payout will also not succeed if the User does not have `KYCLevel` of `REGULAR`.

In both cases, however, the VOP data is still returned in the response to [POST Create a Payout](/api-reference/payouts/create-payout) if it applies to the associated `BankAccountId`. 
</Note>
