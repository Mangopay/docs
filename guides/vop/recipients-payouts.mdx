...
- [POST Create a Payout](/api-reference/payouts/create-payout), if the owner of the payout’s `BankccountId` has at least one User-Owned Virtual Account
...
Mangopay is introducing VOP checks when your platform requests a payout using [POST Create a Payout](/api-reference/payouts/create-payout).
...
When Mangopay performs the VOP check with the Responding PSP, if the result is `MATCH` or `NO_MATCH`, then the payout will continue as before.
