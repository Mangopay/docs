...
Call the <a href="/api-reference/recipients/view-payout-methods" class="http-get">GET View payout methods</a> endpoint to find out which payout methods are available to your platform in each currency and destination country.
</Step>
...
Check that the user's data is valid by submitting it to the <div class="rounded-lg font-bold px-1.5 py-0.5 text-sm leading-5 bg-blue-400/20 text-blue-700 dark:bg-blue-400/20 dark:text-blue-400">POST</div>[POST Validate data for a Recipient](/api-reference/recipients/validate-recipient-data) endpoint.
