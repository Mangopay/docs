...
To register a bank account as a Recipient, the user must authenticate directly with Mangopay via SCA. This means the user must be have the `UserCategory` `OWNER` and be [enrolled in SCA](/guides/sca) beforehand.

During the Recipient registration, you need to redirect the user to a Mangopay-hosted webpage to complete an [SCA session](/guides/sca/session) (Step 5 below).
</Note>
...
The user must complete [SCA](/guides/sca) to register their bank account. This means you need to redirect them to the Mangopay-hosted SCA session. For more details on this process, see the [SCA session](/guides/sca/session) guide.
