...
Starting Monday, September 15, 2025, Mangopay will categorize existing and newly created Banking Aliases by adding the `VirtualAccountPurpose` field to the object. Once the categorization is performed, it is final and cannot be changed. 
...
Mangopay is adding fields to the API response on the existing [POST Create a Payout](/api-reference/payouts/create-payout) endpoint to return the result of the VOP check on each payout request. These new fields will also be added on [recipient creation](#vop-on-recipients-bank-accounts) to return the result of the VOP check performed on recipients.

There are three new data points, which will be returned inside a new `RecipientVerificationOfPayee` JSON object parameter: 
...
For the active Bank Account and Recipient objects that have already been created, Mangopay will provide a dedicated endpoint that platforms can call to retrieve the VOP result and `RecipientVerificationOfPayee` data.
