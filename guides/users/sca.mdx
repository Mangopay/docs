...
Mangopay is implementing **strong customer authentication** (SCA) on your platform's users to enhance security and to comply with European regulations (PSD2) as a regulated [EMI](/guides/glossary#electronic-money-institution-emi).
...
The table below lists the actions that will be authenticated by SCA. As indicated, some are available to integrate. For others, the anticipated release date is given.
...
<td class="table-content">
Change of SCA credentials
</td>
<td class="table-content">
When an SCA-enrolled user changes their phone number or email they must re-enroll the new credentials.
</td>
</tr>
<tr>
<td class="table-content">
...
Mangopay is releasing a new version of the Bank Account object called **Recipients**. Recipients will allow validation of account details, supporting faster and more reliable local and international payouts.

Registering a Recipient will require SCA.
</td>
</tr>
<tr>
<td class="table-content">
...
<td class="table-content">
Transfers from one `OWNER` user to another (between Mangopay account holders) are in scope of SCA. The conditions surrounding transfers are still being defined and Mangopay is developing features to support platforms' activity and a range of exemptions. Among those already defined:
...
The new transfer endpoint will include a `UserPresent` parameter which, if set to `false`, will not trigger SCA. The use of the `UserPresent` parameter will be subject to workflow validation and implemented on a case-by-case basis.
</td>
</tr>
<tr>
<td class="table-content">
...
<td class="table-content">
When an Owner user wishes to view their wallet balance, they may be required to authenticate. The conditions surrounding wallet access are still being defined but may include, for example, first-time access. The `UserPresent` feature will be used in a similar way as for transfers.
</td>
</tr>
<tr>
<td class="table-content">
...
<td class="table-content">
When an Owner user wishes to see past transactions crediting or debiting their wallet, they may be required to authenticate. The conditions surrounding wallet access are still being defined, and the `UserPresent` feature will be used in a similar way as for transfers.
</td>
</tr>
</tbody>
</table>
