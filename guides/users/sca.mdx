...
- Obtain a new SCA redirection link by calling [POST Activate a User](/api-reference/users/activate-user), if a previous authentication attempt was unsuccessful or the session timed out
...
If this happens, you need to obtain a new `RedirectUrl` for them using the endpoint [POST Activate a User](/api-reference/users/activate-user).
</Note>
</Step>
...
If the `controlStatus` is **not** `VALIDATED`, call the [POST Activate a User](/api-reference/users/activate-user) to obtain a new SCA `RedirectUrl` and retry the authentication session. 
...
If the `actionStatus` is **not** `SUCCEEDED`, call the [GET View a User](/api-reference/users/view-user) to check the `UserStatus` of the user. If it is still `PENDING_USER_ACTION`, then call the [POST Activate a User](/api-reference/users/activate-user) to retry the authentication. 
</Step>
</Steps>
