...
In many cases, you may need to categorize a set of users as `OWNER`. SCA is being introduced on act of categorizing a `PAYER` as an `OWNER`, as described in [SCA on Users](/guides/sca/users). However, it is still possible to categorize users as `OWNER` via the legacy endpoint and via the Dashboard. In this case, the `OWNER` must later be [enrolled in SCA separately](/guides/sca/users#enroll-an-existing-owner).

In other cases, a more significant change in your integration may be required to align your usage with the restrictions below. We invite you to consult the rest of our documentation to familiarize yourself with the current integration standards, and to contact Support <a href="https://hub.mangopay.com/" target="_blank">via the Dashboard</a> if you require confirmation on any matter or have a query. 
...
Users with `UserCategory` of `PAYER` are not allowed to receive transfers from **other** users. However, `PAYER` users **are** permitted to transfer funds between wallets belonging to them. `PAYER` users can also receive transfer refunds (and pay-in refunds).
