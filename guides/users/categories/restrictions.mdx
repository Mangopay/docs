...
Users with `UserCategory` of `PAYER` are not allowed to receive transfers from **other** users. However, `PAYER` users **are** permitted to transfer funds between wallets belonging to them. `PAYER` users can also receive transfer refunds.
