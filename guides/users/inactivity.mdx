...
To cover the maintenance and security of inactive accounts, Mangopay applies an inactivity fee. 

Before charging the fee, Mangopay notifies the Natural or Legal User (Mangopay Account holder) and invites them to perform activity on your platform to demonstrate that they are still active.
...
#### 1. Email notification to the user
...
The notification is sent to the following email addresses:
- [Natural User](/api-reference/users/natural-user-object-sca) – The `Email` property value.
- [Legal User](/api-reference/users/legal-user-object-sca) – Both the `Email` value and the `LegalRepresentative.Email` value (2 emails sent).
...
The inactivity fee is withdrawn monthly, during the month that a user becomes inactive.
...
Every 1 year thereafter, unless the account becomes active by initiating a transaction (pay-in, transfer out, FX conversion, or payout), Mangopay withdraws the same fee (30 EUR or equivalent amount in the wallet’s currency, limited by the wallet balance).
