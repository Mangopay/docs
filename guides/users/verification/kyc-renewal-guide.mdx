...
If the user's `LegalPersonType` is `BUSINESS` or `PARTNERSHIP`, submit a [UBO Declaration](/api-reference/ubo-declarations/ubo-declaration-object) for them listing the necessary UBOs.
