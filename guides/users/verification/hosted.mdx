...
The hosted session analyzes the individual’s identity document and extracts its first name, last name, and date of birth. These three data points must correspond to data that is already present in the user object before the session is initiated. When a session is successful, these verified data points are overwritten in the user object (see [verified data](#verified-data) for details).

For Natural users, the first name, last name, and date of birth of the identity document must match the `FirstName`, `LastName`, and `Birthday` of the Natural object.

For Legal Soletrader users, then the first name, last name, and date of birth of the identity document must match the `FirstName`, `LastName`, and `Birthday` of the `LegalRepresentative` object of the corresponding Legal user.
...
| `IDV_NAME_MATCH_CHECK`  | Checks whether the first name, last name and date of birth of the identity document match the `FirstName`, `LastName`, and `Birthday` of the Natural user object. <br/> <br/> Only performed if `IDENTITY_DOCUMENT_VERIFICATION` is successful. |
...
| `IDENTITY_DOCUMENT_VERIFICATION` | Verifies the authenticity of the identity document, the liveness of the selfie, and that the selfie matches the identity document photo. |
| `IDV_NAME_MATCH_CHECK` | Checks whether the first name, last name and date of birth of the identity document match the `FirstName`, `LastName`, and `Birthday` of the `LegalRepresentative` of the Legal user object. <br/> <br/> Only performed if `IDENTITY_DOCUMENT_VERIFICATION` is successful.  |
...
<td class="table-content">`DOCUMENT_NOT_ACCEPTED`</td>
<td class="table-content">The name and date of birth of the identity document did not match with the `FirstName`, `LastName` and `Birthday` of the natural user or the declared `LegalRepresentative` in the user object.</td>
<td class="table-content">
