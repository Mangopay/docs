...
Mangopay's hosted KYC/KYB verification solution allows platforms to redirect a user to a secure Mangopay-hosted web experience where they can submit their details and documents for identity and business verification.
...
The user must also be an Owner to request a verification session. To verify a user, you need to create a session. Each session contains multiple checks depending on the user type.
</Note>
...
See details of a session's status and verified data
</Card>
<Card title="GET List IDV Session for a User" href="/api-reference/idv-sessions/list-idv-sessions-user">
Retrieve key details of all IDV Sessions attempted for a user
</Card>
</CardGroup>
