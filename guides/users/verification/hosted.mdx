...
<Note>
**Note – Beta hosted identity verification for Legal Soletraders and Businesses**

The hosted identity verification feature is in beta and only available for Legal users whose `LegalPersonType` is `SOLETRADER` or `BUSINESS`. For more information about all user types, see the [Introduction and Types](/guides/users/types) guide.

The user must also be an Owner to request a new verification session. To verify a user, you need to create a session. Each session contains multiple checks depending on the legal entity type.
</Note>
...
<table>
    <tbody>
        <tr>
            <td class="table-content">[POST /users/&lcub;UserId&rcub;/identity-verifications](/guides/users/verification/hosted#post-create-a-hosted-identity-verification-session)</td>
...
This endpoint allows you to initiate an identity verification session for a user and provides a redirection link to Mangopay's hosted experience. Each session contains multiple checks depending on the legal entity type.
