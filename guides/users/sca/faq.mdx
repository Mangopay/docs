...
SCA will be triggered every time an existing user performs a [SCA-triggering action](/guides/users/sca#actions-requiring-sca), such as registering a Recipient (bank account), initiating a transfer (if `UserPresent` is true), or viewing their wallet balance and transaction history. The dedicated [enrollment endpoint](/api-reference/users/enroll-user) allows your platform to conveniently introduce SCA enrollment for existing users in your existing user journey.
