---
title: SCA enrollment during Owner registration
...
4. [Re-enroll a user](#) if they change their credentials
...
- Set `UserCategory` to `OWNER`
- Set `TermsAndConditionsAccepted` to `true` (ensuring you have obtained the user's acceptance of Mangopay's T&Cs)
- Set the `LegalPersonType` as `SOLETRADER` (according to [local structure](/guides/users/verification/requirements/kyb-local)) 
- Provide the required data for Owners
...
### Re-enroll an Owner user

Re-enrollment is required when the `PhoneNumber`, `PhoneNumberCountry`, or `Email` are modified.
...
Payers are not subject to SCA. However, we strongly recommend that you implement the new SCA-enabled endpoints below to register new Payers and update existing ones, regardless of whether they are likely to become Owners in the future.
