...
Depending on your workflow, you can do this at [creation](#creation) or using the dedicated endpoints to [transition](#transition) an existing Payer into an Owner.
...
#### Legal User
- `LegalPersonType` (according to [local structure](/guides/users/verification/requirements/kyb-local))
...
<Card title="POST Enroll a User" href="/api-reference/users/enroll-user">
Obtain a new SCA redirection link to authenticate a user
</Card>

## View users

The following endpoint is also available for managing users:
