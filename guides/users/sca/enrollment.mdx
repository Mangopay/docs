...
import ScaUserLifecycle from '/snippets/icons/diagram_SCA_user_lifecycle.mdx';
...
The unique page URL for the SCA session is returned in the `RedirectURL` value of the `PendingUserAction` response parameter.

The `UserStatus` response parameter is `PENDING_USER_ACTION`, showing that the user needs to enroll in SCA and authenticate before they can access Mangopay features as an Owner.
</Step>
</Steps>

### SCA scenarios

The need for a user to enroll in SCA is indicated by the response parameter `UserStatus`. The values are:
...
To allow a user to enroll in SCA, you must redirect them to the SCA session on the returned `RedirectURL` as described in the [How to redirect a user for SCA](guides/users/sca#how-to-redirect-a-user-for-sca) section.
...
<img src="/images/diagram_SCA_user_lifecycle.svg" />
