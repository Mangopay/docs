...
If you have questions that aren't addressed in the pages below, or in our [dedicated FAQ](/guides/users/sca/faq), please get in touch with our teams <a href="https://hub.mangopay.com/" target="_blank">via the Dashboard</a>.
</Info>
...
The table below lists the integration that we require your platform to undertake, along with their availability in Sandbox to start implementation.
...
<tr>
  ...
  New user-creation endpoint for enrollment when you [register a Owner user for the first time](/guides/users/sca/enrollment#register-an-owner-for-the-first-time) (Natural and Legal, all types).
  </td>
  <td class="table-content">
  Now
  </td>
...
<tr>
  <td class="table-content">
  New user-categorization endpoint for enrollment when you [transition a Payer to Owner](/guides/users/sca/enrollment#transition-a-payer-to-owner) (Natural and Legal, all types)
  </td>
  <td class="table-content">
  Now
  </td>
...
<tr>
  <td class="table-content">
  New user-modification endpoint to [re-enroll an enrolled Owner](/guides/users/sca/enrollment#re-enroll-an-enrolled-owner) if contact details changed (Natural and Soletrader).
  </td>
  <td class="table-content">
  Now
  </td>
...
<tr>
  <td class="table-content">
  New endpoint to [enroll an existing Owner](/guides/users/sca/enrollment#enroll-an-existing-owner) (Natural and Soletrader) at an appropriate moment in your user journey
  </td>
  <td class="table-content">
  Now
  </td>
...
<td class="table-content">
See Legal user integration, date to be announced
</td>
</tr>
<tr>
<td class="table-content">
SCA on Recipients (bank account/payment account registration), including webhooks	
</td>
<td class="table-content">
Mid-April
</td>
...
<tr>
<td class="table-content">
SCA on Transfers (user-initiated between Owners) using ScaContext	
</td>
<td class="table-content">
Mid-April
</td>
...
<tr>
<td class="table-content">
SCA on Mangopay Account access (wallet balance and transactions history)	
</td>
<td class="table-content">
Mid-May
</td>
...
Exemptions are not automatic. Mangopay must justify them with sufficient evidence relating to each specific request.
...
## Actions not impacted by Mangopay's SCA
...
Note that if you change a `PAYER` to `OWNER` via the Dashboard, that action uses the legacy **PUT Update a User** endpoint, and as such it will not trigger SCA, even if the `PAYER` was created using the new SCA-enabled endpoint. Such as user will still need to enroll as an existing user via the [POST Enroll a User](/api-reference/users/enroll-user) endpoint.
