...
The SCA-controlled actions impact `OWNER` users at certain moments of their activity with Mangopay.
...
SCA authentication therefore impacts the recipient creation endpoint, the transfer endpoint, and 4 endpoints for account access.
...
The SCA regulations allow Mangopay to apply an exemption to a given instance of an SCA-controlled action. The exemptions that Mangopay can apply depend on the action being authenticated. 
...
SCA is applicable to all the [SCA-controlled actions](#actions-requiring-sca) that the `OWNER` user takes, whether via the API or via the Dashboard. Initially, your platform needs to change its API implementation. 

Mangopay will be releasing features to address Dashboard-based actions.
...
Initially, SCA will not be applied to a User whose `UserCategory` is `PLATFORM`. SCA is applicable to actions taken by these users, however, and Mangopay is defining a suitable solution.
