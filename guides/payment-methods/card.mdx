...
Processing card payments with Mangopay involves passing sensitive card details to a dedicated tokenaization server to generate a `CardId`.
...
<Steps>
    ...
    <Step title="Tokenize the card">
        Send the cardholder's sensitive data directly to the tokenization server, without passing yours or Mangopay's.
    </Step>
    <Step title="Update the card registration object">
        Send the `RegistrationData` received from the tokenization server to Mangopay to generate a `CardId`. 
    </Step>
...
For more detailed guidance, see the <a href="/guides/payment-methods/card/direct">one-time payment guides</a>.
