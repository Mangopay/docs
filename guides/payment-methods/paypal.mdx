...
<Steps>
<Step title="Submit data about the future ">
...
Your platform submits data about the transaction and retrieves the returned `DataCollectionId` ([see above](#submit-pre-transaction-data)).
</Step>
...
Send data about the transaction and retrieve the returned `DataCollectionId` ([see above](#submit-pre-transaction-data)).
</Step>
...
Each subsequent payment, without the user present, also requires a new `DataCollectionId` as in the steps above.
