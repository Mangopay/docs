...
<Note>
    **Note - Card validation not returned in Transactions** A card validation is
    a request for authentication only: it does not request capture or debiting
    of any funds. Therefore, it is not stored as a Transaction in the API.
</Note>
...
<Note>
    **Note - 3DS still possible on payments after card validation** The card
    validation feature makes it possible to validate a card that has been
    registered. It does not change the likelihood of SCA being applied again at
    the moment of payment with the card.
</Note>

## Related resources

<CardGroup col={2}> 
    <Card title="How to" href="/how-to/validate-card-without-debit">Learn how to validate a card without debiting a payment</Card>  
