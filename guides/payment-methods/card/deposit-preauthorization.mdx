...
<Steps>
  ...
  <Step title="Capture pay-in">
    Capture the preauthorized funds using the dedicated <a href="/api-reference/deposit-preauthorizations/create-deposit-preauthorized-payin">POST Create a Deposit Preauthorized PayIn without complement</a> endpoint
  </Step>
</Steps>

This scenario is described step by step in the tutorial:

<a href="/guides/payment-methods/card/deposit-preauthorization/how-to">How to process a 30-day preauthorization →</a>
...
<Steps>
  ...
  <Step title="Request no-show">
    Request the no-show, which also signals the intent to capture an additional amount, using the <a href="/api-reference/deposit-preauthorizations/cancel-deposit-preauthorization">PUT Cancel a Deposit Preauthorization or request a no-show</a> endpoint
  </Step>
