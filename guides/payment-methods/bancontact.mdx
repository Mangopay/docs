...
On a website, you redirect the user to a hosted payment page via the `RedirectURL`. 
...
<Note>
  **Note – Co-branded cards not supported**

  The Bancontact pay-in flow does not support co-branded cards. If attempted, the user will be prompted to retry with a card that is only branded Bancontact. 
</Note>
