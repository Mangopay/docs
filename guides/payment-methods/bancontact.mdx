...
You create the payment request by calling <a href="/api-reference/bancontact/create-bancontact-payin">POST Create a Bancontact PayIn</a>.

For an app-to-app flow, you need to set the `PaymentFlow` to `APP` to bring the user back to your platform's app (otherwise `WEB` for browser-based by default).
</Step>
...
<Step title="Return">
After payment, the user is returned:
- To your specified `ReturnURL` if the `PaymentFlow` is `WEB` (default value)
- To your platform's app if the `PaymentFlow` is `APP`
</Step>
<Step title="Outcome">
