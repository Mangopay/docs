...
For SDD, Mangopay only supports the SEPA Core scheme and not the SEPA B2B scheme. Furthermore, the IBAN must be accepted as part of the SEPA Core scheme. Some European countries, notably Denmark and Sweden, operate different local direct debit schemes and IBANs from these countries (starting `DK` or `SE`) may not be supported.
...
For SDD, IBANs that are not SEPA Core are not accepted ([read more](#iban-compatibility)).
</Step>
