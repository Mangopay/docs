...
<table>
    ...
    <tbody>
        ...
        <tr>
            <th class="header">Email notification</th>
...
## Emails and other notes
The emails mentioned in the tables above are sent by Mangopay (to the user’s `Email` parameter) to notify the user of the payment amount, the charge date, and other details about the collecting party. While these emails are not customizable, it is possible to deactivate them so you can put in place your own flow.

Note that for Bacs, there is an additional email sent when the mandate is first confirmed.
