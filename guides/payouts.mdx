...
If the payout is in EUR in the SEPA zone, Mangopay offers two other modes which platforms can specify in the `PayoutModeRequested` field of the [POST Create a Payout](/api-reference/payouts/create-payout) endpoint: 
...
- **Real-time gross settlement (RTGS)** – Sent via the Eurozone’s RTGS scheme, T2 (formerly TARGET2), meaning funds are usually processed within 5 minutes. If the request is received after the applicable cutoff (16:15 CET), it is processed the next working day (from 07:00 CET). For more details, see [RTGS](/#) below.
