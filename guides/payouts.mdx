...
Non-domestic rails rely on the global shared messaging service SWIFT to transfer funds. SWIFT transfers can take between 3 and 5 days to reach their destination.
...
Legacy bank accounts, created before the Recipient feature was released, were migrated to recipients and assigned a `Country`.
...
Legacy Bank Account objects had no currency property, and so they could be used for payouts in multiple currencies. The Recipients feature has a `Currency` for each Recipient to improve payout deliverability, meaning that the same real-life account can be registered multiple times.
