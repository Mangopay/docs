...
### Registering the bank account

When you register the user's bank account, you should use the data returned on the pay-in as far as possible (and depending on the [type of bank account](/#bank-accounts) you need to create). You can obtain data via a GET call on the pay-in.

On [GET View a PayIn (Bank Wire)](/api-reference/bank-wire-payins/view-payin-bank-wire), 
