...
For the payout to be correctly processed as a reimbursement, you must reference the initial transaction in the payout request. To do so, use `PaymentRef` body parameter when you call the [POST Create a Payout](/api-reference/payouts/create-payout) endpoint:
