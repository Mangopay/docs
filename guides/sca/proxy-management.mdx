...
In order to be allowed to perform an [SCA-triggering action](/guides/sca#actions-requiring-sca) on a user’s behalf: 
...
For compliance reasons, implementing this feature requires authorization by Mangopay and is not necessarily available for all platforms. If your platform wishes to take advance of proxy for SCA-triggering actions, contact our teams <a href="https://hub.mangopay.com/" target="_blank">via the Dashboard</a>. 
...
[Read more in the provisional guide](/guides/sca/platform) **&rarr;**
...
2. **SCA context**
  ...
  - **The user is taking the action:** they are on-session and able to perform SCA and your platform redirects them to the SCA hosted experience (if SCA redirection link is returned by the API, which doesn’t happen if Mangopay applies an exemption). This is indicated by the `USER_PRESENT` value.
  - **The platform is taking the action under proxy** **from the user** and the user has given consent to Mangopay to allow the action. This is indicated by the `USER_NOT_PRESENT` value.
...
<td class="table-content">Integrate consent management endpoint to collect consent and enable users to revoke it</td>
<td class="table-content">Now</td>
<td class="table-content">ASAP</td>
</tr><tr>
</tr><tr>
<td class="table-content">Start collecting user consent for proxy (via dedicated endpoint and other SCA sessions)</td>
<td class="table-content">Once proxy activated in production</td>
<td class="table-content">ASAP</td>
...
4. Independently of enrollment or an SCA-triggering action, on the SCA session URL returned by the [POST Manage proxy consent for a User](/api-reference/users/manage-proxy-consent) endpoint
...
The user must be able to revoke their consent via your platform at any time. You need to enable this by integrating the [POST Manage proxy consent for a User](/api-reference/users/manage-proxy-consent) endpoint. 
...
The [POST Manage proxy consent for a User](/api-reference/users/manage-proxy-consent) endpoint allows your platform to:
...
- Obtain consent for the new scope during the next SCA session for that user, which can be via [POST Manage proxy consent for a User](/api-reference/users/manage-proxy-consent)
...
- [POST Manage proxy consent for a User](/api-reference/users/manage-proxy-consent) can only be called if the **user is** **already enrolled in SCA** (`UserStatus` is `ACTIVE`)
...
- `USER_NOT_PRESENT` – **The platform is taking the action under proxy from the user** and the user has given consent to Mangopay to allow the action. 
...
<td class="table-content">Before proxy and user consent management, not sending `ScaContext` or sending `USER_NOT_PRESENT` was a temporary measure used to allow platforms to adopt SCA.</td>
</tr>
<tr>
...
<td class="table-content">
- [PUT Update a Natural User (SCA)](/api-reference/users/update-natural-user-sca) (available)
- [PUT Update a Legal User (SCA)](/api-reference/users/update-legal-user-sca) (available)
</td>
<td class="table-content">
...
<td class="table-content">
- [POST Create a Recipient](/api-reference/recipients/create-recipient) (release planned early November 2025)
</td>
<td class="table-content">
...
{/* light mode */}
<div className="block dark:hidden w-3/4 mx-auto">
```mermaid
%%{
  init: {
    'theme': 'base',
...
{/* dark mode */}
<div className="hidden dark:block w-3/4 mx-auto">
```mermaid 
%%{
  init: {
    'theme': 'base',
...
It may happen that Mangopay can apply an [SCA exemption](/guides/sca#exemptions-available-to-mangopay) on a specific instance of an SCA-triggering **transfer** or **wallet access** request. If this happens, then the `PendingUserAction.RedirectUrl` is not returned even if the `USER_PRESENT` value was sent for `ScaContext`.
...
```json 403 - Consent not given for USER_NOT_PRESENT
{
...
1. Obtain consent from the user by calling [POST Manage proxy consent for a User](/api-reference/users/manage-proxy-consent) and redirecting the user to the hosted SCA session, asking them to check the relevant checkbox. Then retry the initial action with `USER_NOT_PRESENT`. 
1. Retry the action with the user on session to authenticate by setting `ScaContext` to `USER_PRESENT` and redirecting the user on the `PendingUserAction.RedirectUrl` returned. Note that this SCA session also presents the user with the consent screen on which they can check the checkbox to give consent for future actions.