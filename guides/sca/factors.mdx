...
Mangopay is delivering SCA via a unique session on a hosted webpage. The URL for the session is returned on relevant API calls, and your platform needs to redirect the user to complete the session – see [SCA session](/guides/users/sca/session) for details on how to do this.
...
In Sandbox, you can use the phone number `0611111111` and `FR` (`+33611111111`) with the passcode **702100** to simulate a successful flow. You can also send this data in the in the `PhoneNumber` (or `LegalRepresentative.PhoneNumber`) to pre-populate the field.
