...
Below is an example of the SCA welcome screen on Owner user creation:
...
Note that the authentication example above is for a [transfer](/guides/sca/transfers) request, which shows the transfer currency and amount. The trading name is shown in every scenario along with:
...
Defining a trusted device as a passkey is optional. 
...
Because WebAuthn doesn't rely on a password, it is resistant to phishing and data breaches. It has wide adoption on web browsers and mobile devices.
