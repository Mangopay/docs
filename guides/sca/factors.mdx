...
Mangopay has integrated it to be a combined possession (of the device) and inherence or knowledge type (depending on the devices authentication. Furthermore, the email confirmation step is not required on authentication (it is still required for enrollment or re-enrollment).

The WebAuthn passkey represents one of the most secure web-based authentication methods and we strong encourage your users to adopt it.
</Check>
...
flowchart LR
    ...
    n4 == No ===> n17["User confirms email"]
    n22["User auths PIN"] ==> n32["User auths (new/unmodified) OTP SMS"]
    n30 ==> n4
