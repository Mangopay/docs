...
#### Is the phone number mandatory for us? How should we handle Related&nbsp;guidesg phone number data?
...
Only modifications to phone number and email require validation through SCA by the end user. SCA has no impact on the Related&nbsp;guidesg KYC processes surrounding [documents](/guides/users/verification) or the [downgrade](/guides/users/verification/downgrade) mechanism.
...
#### We already have SCA or MFA on our website or app. Can we use our Related&nbsp;guidesg feature and send validation to Mangopay through the API?
...
#### When will SCA be triggered for our Related&nbsp;guidesg users, and how will it work?
For Related&nbsp;guidesg users, Mangopay's features allow your platform to enroll them at a convenient point in their journey using the dedicated [POST Enroll a User](/api-reference/users/enroll-user) endpoint. 

SCA will be applied when the user takes an action requiring SCA.

For more details, see [Related&nbsp;guidesg user integration](/guides/sca/users#Related&nbsp;guidesg-user-integration).
...
No. You will be able to continue using Related&nbsp;guidesg bank accounts (`BankAccountId`) for payouts.
...
Initially, Related&nbsp;guidesg users will not be restricted by the introduction of SCA.

Related&nbsp;guidesg users, who already have `ACTIVE` as their `UserStatus`, will not be restricted or transitioned to `PENDING_USER_ACTION` if you attempt to enroll them using [POST Enroll a User](/api-reference/users/enroll-user) and the SCA session is not successful.
...
No, you will not be required to sign any contract amendment – our Related&nbsp;guidesg contract remains in effect.
