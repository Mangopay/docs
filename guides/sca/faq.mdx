...
However, in certain cases we may grant an exemption to this situation if your workflow involves automated user object creation. In such cases, it is essential that your app or website offers an option for your users to complete their SCA enrollment using the dedicated [POST Enroll a User](/api-reference/users/enroll-user) endpoint at the appropriate time.
