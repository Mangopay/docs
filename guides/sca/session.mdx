...
When Mangopay introduced SCA, there was a second query parameter, `controlStatus`, that was added to the `returnUrl` on redirection. This was subsequently removed for consistency across all SCA redirection scenarios, and to encourage reliance on webhooks.
