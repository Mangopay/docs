...
When Mangopay determines that SCA is required, a call to any of the 4 endpoints above will return a **401 - Unauthorized**. The `redirectUrl` value needed for the user to be able authenticate is returned in the `WWW-Authenticate` response header.

Your platform needs to retrieve this value to redirect the user.
