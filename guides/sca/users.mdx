...
A call to the dedicated enrollment endpoint ([POST Enroll a User in SCA](/api-reference/users/enroll-user)) does **not** change the user's status to `PENDING_USER_ACTION` to ensure the user is not blocked if they are unable to complete SCA successfully.
