...
The `PENDING_USER_ACTION` status value indicates that the user needs enroll in SCA.
...
For `PENDING_USER_ACTION` these users, only the SCA-controlled actions are blocked until the user successfully enrolls in SCA. 
</Note>
