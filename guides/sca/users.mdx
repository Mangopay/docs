...
When the user's `UserStatus` is `PENDING_USER_ACTION`, they are unable to take the following actions: 
...
Attempts to perform the actions above return an error. In some cases this is indicated by a 400 response: 
...
On transfers, such a request fails immediately with the dedicated `ResultCode`: 
