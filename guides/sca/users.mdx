...
Your platform needs to use the `RedirectUrl` to redirect the individual to Mangopay's hosted-webpage so they can enroll. See [SCA session](/guides/users/sca/session) for more details on how to do this.
...
Existing users will not be immediately restricted by the introduction of SCA ([see below](#restrictions-on-existing-users)). Mangopay's new endpoints and features allow your platform to integrate SCA without undue impact on your existing users.

For existing users, SCA redirection will be triggered in the following scenarios:

- For existing `PAYER` users, when you categorize them using [PUT Categorize a Natural User](/api-reference/users/categorize-natural-user) or [PUT Categorize a Legal User](/api-reference/users/categorize-legal-user) (available now)
- For existing `OWNER` users, when you:
    ...
    - Request a [user-initiated transfer](/guides/sca/transnfers) for them to another `OWNER` (when SCA feature launched)
    - Request to access their [wallet balance or transaction history](/api-reference/) (when SCA feature launched)
...
- Integrate the new endpoints for user creation. [Read how](/guides/users/sca/enrollment#register-an-owner-for-the-first-time) **&rarr;**
- Integrate the new endpoints for changing a `PAYER` to an `OWNER`. [Read how](/guides/users/sca/enrollment#transition-a-payer-to-owner) **&rarr;**
- Integrate the new endpoints for re-enrollment of enrolled users. [Read how](/guides/users/sca/enrollment#re-enroll-an-enrolled-owner) **&rarr;**
...
- Integrate SCA enrollment existing `OWNER` users at an appropriate moment in your user journey using the dedicated endpoint. [Read how](/guides/users/sca/enrollment#enroll-an-existing-owner) **&rarr;**
</Info>
