...
On the [POST Create a Transfer](/api-reference/transfers/create-transfer) endpoint, Mangopay has introduced the body parameter `ScaContext`, which your platform needs to send on all Owner-initiated transfers. 
...
If Mangopay determines that SCA is required for the transfer request, the API response contains `PendingUserAction.RedirectUrl` and the individual must authenticate on the Mangopay-hosted webpage within 10 minutes of the API response.
...
#### Example of API response if SCA required
...
After the SCA session, the user is returned to your specified `returnUrl`, regardless of the outcome.
...
- Legal user's `LegalPersonType` is `BUSINESS`, `PARTNERSHIP`, or `ORGANIZATION`
- The amount is less than 30 EUR or equivalent
- `ScaContext` is not sent (in which case it is `null`)
