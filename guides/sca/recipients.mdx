...
The Recipients feature will replace the legacy Bank Account endpoints, including their types. 
...
This kind of recipient, with the `RecipientScope` value `PAYIN`, can be created for a user with the `UserCategory` `PAYER`, and does not require SCA.
