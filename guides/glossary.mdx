...
Mangopay interface enabling platform operators to manage their payment operations. Available through the Mangopay Dashboard, the Dashboard is specific to each set of API credentials (Client ID and API key) in both the Production and Sandbox environments.  
...
Mangopay interface to centralize the platform's entire experience with Mangopay. The Hub gives access to test the full API for free as much needed and use the Mangopay Dashboard to manage payment operations. Platforms can create Hub profiles for their teams and centralize communications with our Support. via the Dashboard, platforms can also begin and manage their business relationship with Mangopay. 
