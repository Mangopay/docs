...
Mangopay is developing a new **Pay-In Connector** to provide better transparency on funds acquired by other PSPs but which then transit the Mangopay environment. 

The Mirakl Connector, which your platform is using, will take care of this functionality for all Mirakl Orders, regardless of whether the payments are processed by a third-party PSP or by Mangopay. However, we require you to enrich Mirakl Orders with some data so they can be processed.
...
- Any platform using a third-party PSP for funds acquisition (whether the platform uses the Mirakl Connector or not)
...
- Any platform using a third-party PSP for funds acquisition and using the Mirakl Connector
...
<td class="table-content">***Single values list*** containing the relevant value(s) exactly as they appear in the [supported external providers](/guides/payin-connector/supported-providers)</td>
</tr>
<tr>
...
<td class="table-content">***Single values list*** containing the relevant values exactly as they appear in the [supported payment methods](/guides/payin-connector/supported-payment-methods)</td>
</tr>
</tbody>
</table>
...
The data values sent in the custom fields must match the supported values:
- The `mgp-external-provider-name` value must be one of the [supported external provider](/guides/payin-connector/supported-providers) values
- The `mgp-external-provider-payment-method` value must be one of the [supported payment methods](/guides/payin-connector/supported-payment-methods)
...
<td class="table-content">One of the [supported external providers](/guides/payin-connector/supported-providers).</td>
</tr>
<tr>
<td class="table-content">`mgp-external-processing-date`</td>
<td class="table-content">Yes</td>
...
- When the Pay-In Connector launches: 
  - Sandbox: May 2025
...
The Mangopay Pay-In Connector will needs settlement data to be able to reconcile the funds entering the Mangopay environment with the payment intents that were previously declared.

For Mirakl Connector platforms, the payment intent declarations are created automatically, but the settlement data must be provided by the platform.
...
Upload the file using the dedicated API endpoint (released as part of the Pay-In Connector).
...
In response, the API will return a JSON object containing `SettlementId`, declared amount (calculated), settled amount, status, etc.
</Step>
<Step title="Handle unprocessed data if required">
In case any data from your settlement could not be processed, you may need to reprocess transactions. The nature of this post-processing is still in development.
</Step>
...
The information provided below aims to prepare you as much as possible for your future integration (Sandbox: May 2025). There may be minor changes in some details as final developments are completed.
</Info>
