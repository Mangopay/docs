...
When creating a payout with the "INSTANT_PAYMENT_ONLY" option, the instant payment bank wire is requested and processed within 10 seconds in the same way as for a regular instant payment. However, if an issue is encountered or if some prerequisites are not met, the wallet will be automatically refunded and the payout will not be completed. 
