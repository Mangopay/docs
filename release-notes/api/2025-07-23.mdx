...
The list of actions refused to these users has been refined to include only those impacted by SCA: recipient or bank account registration, payout requests (conditional on SCA on recipients), transfers debiting their wallet, and wallet access requests.

For the full list, see the [Pending user action – Refused actions](/guides/sca/users#refused-actions) guide section.
