...
The `PaymentRef` must be sent on payouts that are reimbursing [bank wire pay-ins](/api-reference/bank-wire-payins/create-bank-wire-payin) and [pay-ins to virtual IBANs](/api-reference/virtual-accounts/view-payin-external-instruction), for which the [Refund](/api-reference/refunds/refund-object) object is not available.
