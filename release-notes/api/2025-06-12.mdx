...
Platforms can now create virtual accounts for local pay-in collection in the USA and Canada. The `US` and `CA` values have been added for the `Country` body parameter on [POST Create a Virtual Account](/api-reference/virtual-accounts/create-virtual-account), allowing platforms to requests these accounts.
