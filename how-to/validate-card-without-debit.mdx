...
- A `CardId` for the end user’s card (`CardType` must be `CB_VISA_MASTERCARD`), obtained through the card registration flow or returned by the Mangopay [Vault SDK or Checkout SDK](https://mangopay.com/docs/sdk) – in Sandbox, we recommend you use the [challenge test card](https://mangopay.com/docs/dev-tools/testing/payment-methods)
