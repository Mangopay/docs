...
sidebarTitle: 'Validate a card without debiting a payment'
...
- A `CardId` for the end user’s card (`CardType` must be `CB_VISA_MASTERCARD`), obtained through the card registration flow or returned by the Mangopay [Vault SDK or Checkout SDK](/sdks/checkout) – in Sandbox, we recommend you use the challenge test card
...
pprint(create_card_validation._data)  
```
</CodeGroup>
...
<Card title="How to" href="/how-to/process-card-payment">Process a card payment</Card>  

<Card title="Guide">Card validation</Card>