...
sidebarTitle: 'Process a 7-day card preauthorization'
...
try {
    ...
    $cardPreauthorization->AuthorId = '204068024';
...
- For an amount equal or less than the preauthorized amount (full or partial capture)  
- Done within 6.5 days of a successful authorization  

Set up hook notifications for the following `EventType` in order to be notified when it is possible or too late to make a pay-in against a preauthorization:  
...
<Card title="Guide" href="/guides/payment-methods/card/preauthorization">7-day preauthorization</Card>
