...
---  
                    ...
                    | **Error message**     | Author is not the Mandate owner                   |
                    |-----------------------|--------------------------------------------|
                    | **Short description** | The pay-in failed because the author is not the mandate owner. |
...
In the example below, find the API response when using the Create a Direct Direct-Debit PayIn endpoint to make a pay-in with an AuthorId different from the Mandate UserId.  
```json API Response
{
    "Id": "163263010",
