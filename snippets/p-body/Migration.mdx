...
**Caution:** Migration is no longer supported. You can only use objects with `Migration` set to `false`. From Feb 1, 2025, the `false` value will be forced on all objects, including existing ones. Before that date, you need to re-create the object. After, you can re-authenticate the same object.
