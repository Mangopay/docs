...
If the `ReportType` is `USER_WALLET_TRANSACTIONS`, each filter can be used on its own or in combination, in which case the report data satisfies both filters.  Combinations generate a report containing all transactions 
...
The `Currency` and `WalletId` cannot be used together. If 
