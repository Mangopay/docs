...
The SCA context of the request, which is required if the `AuthorId` references a user whose `UserCategory` is `OWNER`:
...
**Note:** `ScaContext` is a temporary mechanism to introduce SCA on existing endpoints and workflows. Platforms integrating for the first time should consider the parameter required and send `USER_PRESENT` by default. Read more about [SCA on transfers](/guides/sca/transfers) **&rarr;**
</ResponseField>