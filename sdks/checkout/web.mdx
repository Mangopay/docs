...
When you enable the **Save Card** option, a **Save Card** checkbox appears on the payment form. If the user selects this checkbox, the `SaveCard` flag is set to `true` and passed to the `onCreatePayment` handler. You can then store the card details in your backend for future use.

**Using Saved Cards for Future Payments**
During future checkout sessions, previously saved cards should be included via the `savedCards` array in the card payment method configuration. When a saved card is selected, the SDK automatically uses it for the payment process.

**Handling Card Removal**
If a user decides to remove a saved card, the `onDeactivateSavedCard` callback is triggered. This allows you to remove or deactivate the card from your backend accordingly.
  ...
  paymentMethods: [
    {
      ...
      onCreatePayment: function (event) {
        /*
          ...
          2. If true:
             - Store the newly created CardId in your backend so it can be
               reused next time (by adding it to savedCards).
          3. If false:
             - Call your backend to deactivate the card in MangoPay API.
             - Ensure the card is marked as inactive to prevent future use.
          4. Create the pay-in on your backend with the CardId or payment details.
        */
      }
    }
  ]
