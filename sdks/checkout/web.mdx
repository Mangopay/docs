---
title: 'Checkout Web integration'  
...
<Check>
**Best practice – Check out our example app**
...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td class="table-content"><code>respectPaymentMethodsOrder</code></td>
            <td class="table-content">Boolean</td>
            <td class="table-content">
                Controls how payment methods are ordered and which one is initially active.<br/>
                When <code>true</code>, the checkout renders methods <em>exactly</em> in the order provided by <code>paymentMethods</code> and the first item in that array becomes the initial selection.<br/>
                When <code>false</code> (default), legacy behavior applies: <code>card</code> is rendered first and preferred as the initial selection; other methods follow their original order.<br/>
...
<div>
  <iframe 
    width="100%" 
    height="700" 
    src="https://cdn.swimlanes.io/dist/embeded.html##fZB0a"
  >
  </iframe>
</div>

{/* 
https://swimlanes.io/u/SjGHpmyXGm

Title: Saved cards – How it works

User<--Checkout SDK: Present payment form
User-->Checkout SDK: Enter card details and check or uncheck ***Save card***
User-->Checkout SDK: Submit payment
Checkout SDK -> Platform: Event: onCreatePayment (payment data + SaveCard true/false)
Platform -> Platform: Record SaveCard  value
Platform -> Mangopay API: POST Create PayIn / Preauth (ReturnURL=checkout.mangopay.com)
Mangopay API --> Platform: API response
Platform -> Platform: If SaveCard=true, save card object
Platform -> Mangopay API: If SaveCard=false, PUT Deactivate Card
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="table-content">
            A unique identifier for your merchant account, required to associate the payment with it when processing PayPal pay-ins. 
