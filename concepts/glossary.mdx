...
The bank or payment service provider (PSP) that acquires the funds from an end user's bank (the <glossary:issuer>issuer</glossary>) on behalf of a business (which is often called a merchant). Mangopay acts as an acquirer for platforms using its services, and also partners with other acquirers. 
...
Movement of funds between a user's bank account and their <glossary:wallet>e-wallet</glossary>, either entering the <glossary:mangopay-environment>Mangopay environment</glossary> as a <glossary:pay-in>pay-in</glossary> or leaving as a <glossary:payout>payout</glossary>.  

A bank wire is also known as a wire, credit, or bank transfer, but the term bank wire is adopted at Mangopay to distinguish it from a <glossary:transfer>transfer</glossary>, which occurs between two e-wallets, within the Mangopay environment.  
...
[Read more &#8594;](/concepts/glossary/beneficial-owner) 

#### Bank identification number (BIN)  

A unique set of digits assigned to an <glossary:issuer>issuer</inline-code>. The BIN typically comprises 6 or 8 digits and is used at the start of the card number (also called the primary account number, or PAN) on all cards issued by the bank or financial institution.  
...
When a chargeback happens, Mangopay automatically creates a <glossary:repudiation>repudiation</glossary> for the contested amount and deducts it from the platform's <glossary:repudiation-wallet>Repudiation Wallet</glossary>. Mangopay also creates a <glossary:dispute>dispute</glossary> to manage the liability of the funds, which may or may not be contestable by the platform.  

For more information, see the Disputes article. 

#### Customer-initiated transaction (CIT)  

A transaction made in the presence of the cardholder, subject to <glossary:sca>strong customer authentication (SCA)</glossary> with no exemption possible. In recurring payments, once the initial CIT is successful subsequent <glossary:mit>merchant-initiated transactions (MITs)</glossary> can be processed. 
...
- Quoted : Conversion executed against a <glossary:quote>quote</glossary> that previously locked in the rate  
...
The management of liability for a <glossary:repudiation>repudiation</glossary> amount, corresponding to a <glossary:chargeback>chargeback</glossary>. A dispute may or may not be contestable by the platform.  
...
A type of financial institution authorized by EU regulations to issue, manage, and distribute electronic money (under Directive 2009/110/EC) and provide payment services (under Directive 2015/2366/EU).  

Mangopay is an EMI under Luxembourg law and authorized to provide services to platforms headquartered in the <glossary:eea>EEA</glossary> and the UK. The record of Mangopay's licensing from the Commission de Surveillance du Secteur Financier (CSSF) in Luxembourg can be viewed on the CSSF's website, as well as on the European Banking Authority's EUCLID database (Payment Institutions Register).  

[Read more &#8594;](/concepts/glossary/emi)  
...
The wallet that stores fees taken by the platform on <glossary:transaction>transactions</glossary> in each currency.  
...
The bank or payment service provider (PSP) of the user making a pay-in, from which Mangopay acquires the funds (as an <glossary:acquirer>acquirer</glossary>) when they are credited to the user's wallet.  
...
A term used to refer to the verification of legal entities - their identity, activity, <glossary:legal-representative>legal representative</glossary>, and <glossary:beneficial-owner>beneficial owners</glossary> - as opposed to know your customer (KYC) for individuals.  

See the documentation for more information on types of user and requirements for legal entities. 

#### Know your customer (KYC)  

A term used to refer to the identity verification of natural persons (i.e., individuals), in contrast with know your business (KYB) for legal entities. The term KYC is also sometimes used to refer to identity verification more generally, without differentiating between individuals and legal entities.  
...
[Read more &#8594;](/concepts/glossary/legal-representative)  
...
The secure digital space where funds can be moved freely in <glossary:transfer>transfers</glossary> between <glossary:wallet>wallets</glossary>, allowing for flexible payment <glossary:workflow>workflows</glossary> to suit the platform's business model. Funds in the Mangopay environment are held securely in accordance with the regulated safeguarding requirements applicable to licensed electronic money institutions in the European Union.  
...
[Read more &#8594;](/concepts/glossary/marketplace) 

#### Maximum Frictionless Amount (MFA)  

Transaction amount above which <glossary:sca>strong customer authentication (SCA)</glossary> is always applied.  
...
The number identifying a payment card, also known as the card number. Typically 16 digits (but shorter or longer depending on the network), the PAN is composed of sets of digits which are assigned to issuers and networks. The first set of digits in the PAN is the <glossary:bin>bank identification number (BIN)</glossary>, which can be used to obtain more information about the card and issuer. 

#### Pay-in  

A movement of funds arriving in the Mangopay environment to the <glossary:wallet>wallet</glossary> of the user making the payment. There are different pay-ins for different payment methods. Like all <glossary:transaction>transactions</glossary>, a pay-in can be subject to a <glossary:refund>refund</glossary>. 

#### Payout  

A movement of funds leaving the Mangopay environment to the bank account of a user. Like all <glossary:transaction>transactions</glossary>, a payout can be subject to a <glossary:refund>refund</glossary> but only at Mangopay's initiation.   
...
[Read more &#8594;](/concepts/glossary/psd2) 
...
The reimbursement of a single <glossary:transaction>transaction</glossary>. A refund is linked to the initial transaction, and each type of transaction can be refunded:  
...
[Read more &#8594;](/concepts/glossary/refund) 

#### Repudiation  

The amount corresponding to a <glossary:chargeback>chargeback</glossary> which is withdrawn from the platform's <glossary:repudiation-wallet>Repudiation Wallet</glossary> at the request of the user's issuing bank.  

The repudiation amount is settled during a <glossary:dispute>dispute</glossary>, which may or may not be contestable by the platform. If the dispute is lost (or was never contestable) then the repudiation represents a debt towards Mangopay that the platform must settle.  

For more information, see the Disputes article. 

#### Repudiation Wallet  

The wallet used to manage funds relating to <glossary:chargeback>chargebacks</glossary> and <glossary:dispute>disputes</glossary>, showing the balance of the platform's chargeback debt with Mangopay. For more information, see the Disputes article.  
...
[Read more &#8594;](/concepts/glossary/sca) 
...
One of three movements of funds in the Mangopay environment: a pay-in (entering), a transfer (within, i.e., wallet to wallet), a payout (leaving). Transactions can be subject to a <glossary:refund>refund</glossary>, which is a dedicated type of transaction linked to the initial transaction. 

#### Transfer  

A movement of funds from one <glossary:wallet>wallet</glossary> to another, within the Mangopay environment.  

In the API, the term transfer always refers to an internal movement of funds; the term bank wire refers to what is often called a bank transfer or wire transfer (which may be a <glossary:pay-in>pay-in</glossary> or <glossary:payout>payout</glossary>).  

## U 

#### User  

A private individual (natural person) or legal entity (legal person) registered with Mangopay to make payments on a <glossary:platform>platform</glossary>. All types and categories of users must provide a minimum of information and comply with further verification requirements depending on the actions they wish to take via the platform.  

Platforms create wallets for users to hold and manage funds. Users may need to register cards, bank accounts, and other information to make <glossary:payin>pay-ins</glossary> and <glossary:payout>payouts</glossary>.  
