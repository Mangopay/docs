...
<table>
    <tbody>
        ...
        <tr>
            ...
            <td class="table-content">The transfer failed because the user's `UserStatus` is `PENDING_USER_ACTION`.</td>
        </tr>
        <tr>
...
This error occurs on the [POST Create a Transfer](/api-reference/transfers/create-transfer) endpoint if the `AuthorId` or the owner of the `DebitedWalletId` has the `UserStatus` of `PENDING_USER_ACTION`. 

This error occurs regardless of the `ScaContext` value sent in the request.

For more information on when this `UserStatus` occurs, see the [Pending user action](/guides/sca/users#pending-user-action) guide section.
