...
This error is triggered on a [pay-in refund](/api-reference/refunds/create-refund-payin) if the initial pay-in's `PaymentType` is `DIRECT_DEBIT` and its `CreationDate` was in the last 7 days.
