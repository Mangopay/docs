...
Call the [POST Create a Report](/api-reference/reporting/create-report) endpoint to generate your report, selecting the `ReportType` and date range (in `AfterDate` and `BeforeDate`).

You should apply the relevant `Filters` to your report. You can also specify the `Columns` your wish to include in the report, otherwise the default ones are used (see tables below).
</Step>
...
To filter the data that will be included in the report, include the relevant `Filters` in the body of your generation request. The available values for each `ReportType` is given below. 

When you generate a report, you can also specify in the `Columns` parameter the columns to be included from those available below. Otherwise, the default columns will be used as described below.

### COLLECTED_FEES

#### Filters (COLLECTED_FEES)
...
#### Filters (USER_WALLET_TRANSACTIONS)

<Filters_Reporting_USER_WALLET_TRANSACTIONS/>

#### Columns (USER_WALLET_TRANSACTIONS)
...
#### Filters (ECHO_INTENT_ACTION)

<Filters_Reporting_ECHO_INTENT_ACTION/>

#### Columns (ECHO_INTENT_ACTION)
...
#### Filters (ECHO_SETTLEMENT)

<Filters_Reporting_ECHO_SETTLEMENT/>

#### Columns (ECHO_SETTLEMENT)
