---
title: 'Create a Satispay PayIn'  
api: 'POST /v2.01/{ClientId}/payins/payment-methods/satispay'  
---

import Country3Response from '/snippets/p-response/Country3Response.mdx';
import StatementDescriptorResponse from '/snippets/p-response/StatementDescriptorResponse.mdx';
import ReturnURLResponse from '/snippets/p-response/ReturnURLResponse.mdx';
import ExecutionTypeResponse from '/snippets/p-response/ExecutionTypeResponse.mdx';
import PaymentType_Response_Satispay from '/snippets/p-response/PaymentType_Response_Satispay.mdx';
import CreditedUserIdResponse from '/snippets/p-response/CreditedUserIdResponse.mdx';
import CreditedWalletIdResponse from '/snippets/p-response/CreditedWalletIdResponse.mdx';
import NatureResponse from '/snippets/p-response/NatureResponse.mdx';
import TypeResponse from '/snippets/p-response/TypeResponse.mdx';
import ExecutionDateResponse from '/snippets/p-response/ExecutionDateResponse.mdx';
import ResultMessageResponse from '/snippets/p-response/ResultMessageResponse.mdx';
import ResultCodeResponse from '/snippets/p-response/ResultCodeResponse.mdx';
import Status2Response from '/snippets/p-response/Status2Response.mdx';
import Fees1Response from '/snippets/p-response/Fees1Response.mdx';
import CreditedFunds2Response from '/snippets/p-response/CreditedFunds2Response.mdx';
import DebitedFunds2Response from '/snippets/p-response/DebitedFunds2Response.mdx';
import AuthorIdResponse from '/snippets/p-response/AuthorIdResponse.mdx';
import CreationDateResponse from '/snippets/p-response/CreationDateResponse.mdx';
import TagResponse from '/snippets/p-response/TagResponse.mdx';
import IdResponse from '/snippets/p-response/IdResponse.mdx';
import ProfilingAttemptReference from '/snippets/p-body/ProfilingAttemptReference.mdx';
import Country3 from '/snippets/p-body/Country3.mdx';
import StatementDescriptor from '/snippets/p-body/StatementDescriptor.mdx';
import Tag from '/snippets/p-body/Tag.mdx';
import ReturnURL from '/snippets/p-body/ReturnURL.mdx';
import Fees1 from '/snippets/p-body/Fees1.mdx';
import DebitedFunds2 from '/snippets/p-body/DebitedFunds2.mdx';
import CreditedWalletId from '/snippets/p-body/CreditedWalletId.mdx';
import AuthorId from '/snippets/p-body/AuthorId.mdx';

import RestCreateSatispayPayin from '/snippets/code/rest-create-satispay-payin.mdx';
import JavaCreateSatispayPayin from '/snippets/code/java-create-satispay-payin.mdx';
import PythonCreateSatispayPayin from '/snippets/code/python-create-satispay-payin.mdx';
import NetCreateSatispayPayin from '/snippets/code/net-create-satispay-payin.mdx';

<Note>  
**Note – Timeout after 30 minutes** 
  
The payment session lasts for 30 minutes, at which point the pay-in fails automatically if no action has been taken by the user.  
</Note>  

### Body parameters  

<AuthorId />  

<CreditedWalletId />  

<DebitedFunds2 />  

<Fees1 />  

<ReturnURL />  

<Tag />  

<StatementDescriptor />  

<Country3 />  

<ProfilingAttemptReference />  

### Responses  

<AccordionGroup><Accordion title="200 - Created">  

<IdResponse />  

<TagResponse />  

<CreationDateResponse />  

<AuthorIdResponse />  

<DebitedFunds2Response />  

<CreditedFunds2Response />  

<Fees1Response />  

<Status2Response />  

<ResultCodeResponse />  

<ResultMessageResponse />  

<ExecutionDateResponse />  

<TypeResponse />  

<NatureResponse />  

<CreditedWalletIdResponse />  

<CreditedUserIdResponse />  

<PaymentType_Response_Satispay />  

<ExecutionTypeResponse />  

<ReturnURLResponse />  

<StatementDescriptorResponse />  

<Country3Response /></Accordion></AccordionGroup>  

<ResponseExample>  

```json 200 - Created  

{
    "Id": "wt_e7a7e499-ed07-4086-a8f1-73207e497b3c",
    "Tag": "Created using the Mangopay API Postman collection",
    "CreationDate": 1707772706,
    "AuthorId": "213407540",
    "DebitedFunds": {
        "Currency": "EUR",
        "Amount": 1000
    },
    "CreditedFunds": {
        "Currency": "EUR",
        "Amount": 1000
    },
    "Fees": {
        "Currency": "EUR",
        "Amount": 0
    },
    "Status": "CREATED",
    "ResultCode": null,
    "ResultMessage": null,
    "ExecutionDate": null,
    "Type": "PAYIN",
    "Nature": "REGULAR",
    "CreditedWalletId": "214818911",
    "CreditedUserId": "213407540",
    "PaymentType": "SATISPAY",
    "ExecutionType": "WEB",
    "ReturnURL": "https://www.mangopay.com/docs/please-ignore?transactionId=wt_e7a7e499-ed07-4086-a8f1-73207e497b3c",
    "RedirectURL": "https://r3.girogate.de/ti/dumbdummy?tx=140186246851&rs=pJm8rdr5445tSOGD5vWL5HPkSDbt7wYR&cs=3726012989f6fc7cbefca757f6f109dc4b06fdf17c29a48c9c88abf1ab5c22f7",
    "StatementDescriptor": "MGP",
    "Country": "FR"
}  
 ```  

</ResponseExample>  

<RequestExample>  

    <RestCreateSatispayPayin />
    <JavaCreateSatispayPayin />
    <PythonCreateSatispayPayin />
    <NetCreateSatispayPayin />
    
</RequestExample>