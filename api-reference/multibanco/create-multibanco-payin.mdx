---
title: 'Create an Multibanco PayIn'  
api: 'POST /v2.01/{ClientId}/payins/payment-methods/multibanco'  
---

import StatementDescriptorResponse from '/snippets/p-response/StatementDescriptorResponse.mdx';
import RedirectURL1Response from '/snippets/p-response/RedirectURL1Response.mdx';
import ReturnURL1Response from '/snippets/p-response/ReturnURL1Response.mdx';
import ExecutionTypeResponse from '/snippets/p-response/ExecutionTypeResponse.mdx';
import PaymentType_Response_Multibanco from '/snippets/p-response/PaymentType_Response_Multibanco.mdx';
import CreditedUserIdResponse from '/snippets/p-response/CreditedUserIdResponse.mdx';
import CreditedWalletIdResponse from '/snippets/p-response/CreditedWalletIdResponse.mdx';
import NatureResponse from '/snippets/p-response/NatureResponse.mdx';
import TypeResponse from '/snippets/p-response/TypeResponse.mdx';
import ExecutionDateResponse from '/snippets/p-response/ExecutionDateResponse.mdx';
import ResultMessageResponse from '/snippets/p-response/ResultMessageResponse.mdx';
import ResultCodeResponse from '/snippets/p-response/ResultCodeResponse.mdx';
import Status2Response from '/snippets/p-response/Status2Response.mdx';
import Fees1Response from '/snippets/p-response/Fees1Response.mdx';
import CreditedFunds2Response from '/snippets/p-response/CreditedFunds2Response.mdx';
import DebitedFunds2Response from '/snippets/p-response/DebitedFunds2Response.mdx';
import AuthorIdResponse from '/snippets/p-response/AuthorIdResponse.mdx';
import CreationDateResponse from '/snippets/p-response/CreationDateResponse.mdx';
import TagResponse from '/snippets/p-response/TagResponse.mdx';
import IdResponse from '/snippets/p-response/IdResponse.mdx';
import ProfilingAttemptReference from '/snippets/p-body/ProfilingAttemptReference.mdx';
import StatementDescriptor from '/snippets/p-body/StatementDescriptor.mdx';
import ReturnURL1 from '/snippets/p-body/ReturnURL1.mdx';
import CreditedWalletId from '/snippets/p-body/CreditedWalletId.mdx';
import Fees1 from '/snippets/p-body/Fees1.mdx';
import DebitedFunds2 from '/snippets/p-body/DebitedFunds2.mdx';
import Tag from '/snippets/p-body/Tag.mdx';
import AuthorId from '/snippets/p-body/AuthorId.mdx';

import CreateMultibancoPayinRest from '/snippets/code/create-multibanco-payin-rest.mdx';
import JavaCreateMultibancoPayin from '/snippets/code/java-create-multibanco-payin.mdx';
import PythonCreateMultibancoPayin from '/snippets/code/python-create-multibanco-payin.mdx';
import NetCreateMultibancoPayin from '/snippets/code/net-create-multibanco-payin.mdx';

<Note>  
**Note – Timeout after 7 days** 
  
The payment session lasts for 7 days, at which point the pay-in fails automatically if no action has been taken by the user.  
</Note>  

### Body parameters  

<AuthorId />  

<Tag />  

<DebitedFunds2 />  

<Fees1 />  

<CreditedWalletId />  

<ReturnURL1 />  

<StatementDescriptor />  

<ProfilingAttemptReference />  

### Responses  

<AccordionGroup><Accordion title="200 - Created">  

<IdResponse />  

<TagResponse />  

<CreationDateResponse />  

<AuthorIdResponse />  

<DebitedFunds2Response />  

<CreditedFunds2Response />  

<Fees1Response />  

<Status2Response />  

<ResultCodeResponse />  

<ResultMessageResponse />  

<ExecutionDateResponse />  

<TypeResponse />  

<NatureResponse />  

<CreditedWalletIdResponse />  

<CreditedUserIdResponse />  

<PaymentType_Response_Multibanco />  

<ExecutionTypeResponse />  

<ReturnURL1Response />  

<RedirectURL1Response />  

<StatementDescriptorResponse /></Accordion></AccordionGroup>  

<ResponseExample>  

```json 200 - Created  

{
    "Id": "wt_6a417de9-5f14-4fed-a8f5-abd7faab21a1",
    "Tag": "Created using the Mangopay API Postman collection",
    "CreationDate": 1696942514,
    "AuthorId": "204068024",
    "DebitedFunds": {
        "Currency": "EUR",
        "Amount": 1000
    },
    "CreditedFunds": {
        "Currency": "EUR",
        "Amount": 1000
    },
    "Fees": {
        "Currency": "EUR",
        "Amount": 0
    },
    "Status": "CREATED",
    "ResultCode": null,
    "ResultMessage": null,
    "ExecutionDate": null,
    "Type": "PAYIN",
    "Nature": "REGULAR",
    "CreditedWalletId": "204089031",
    "CreditedUserId": "204068024",
    "PaymentType": "MULTIBANCO",
    "ExecutionType": "WEB",
    "ReturnURL": "http://www.mangopay.com/docs/please-ignore?transactionId=wt_6a417de9-5f14-4fed-a8f5-abd7faab21a1",
    "RedirectURL": "https://r3.girogate.de/ti/dumbdummy?tx=2137942516&rs=gsc5nItfY1gGGgcNnp1DsiioN90OC23j&cs=198796349738e9ee004aa2e45f3a0e3ee1b49f83927253c15080e8ac1120c207",
    "StatementDescriptor": "Mangopay"
}  
 ```  

</ResponseExample>  

<RequestExample>  
    <CreateMultibancoPayinRest />
    <JavaCreateMultibancoPayin />
    <PythonCreateMultibancoPayin />
    <NetCreateMultibancoPayin />
</RequestExample>