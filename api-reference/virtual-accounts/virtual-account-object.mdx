...
- `USER_OWNED` – Owned by the wallet owner, enabling them to accept and store funds and make payments. KYC verification is required and the user must be categorized as an Owner. Offering User-Owned Accounts requires a contract amendment and activation – contact our teams <a href="https://hub.mangopay.com/">via the Dashboard</a> to get started.
...
Existing platforms are invited to change their integration to benefit from extended country and currency coverage, as well as any future product improvements. Note also that offering User-Owned Virtual Accounts requires a contract amendment.
