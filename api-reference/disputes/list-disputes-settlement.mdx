---
title: 'List Disputes pending settlement'  
api: 'GET /v2.01/{ClientId}/disputes/pendingsettlement'  
---

import Array_DisputesResponse from '/snippets/p-response/Array_DisputesResponse.mdx';

import NodeListDisputesPendingSettlement from '/snippets/code/node-list-disputes-pending-settlement.mdx';
import JavaListDisputesPendingSettlement from '/snippets/code/java-list-disputes-pending-settlement.mdx';
import ListDisputesPendingSettlementPython from '/snippets/code/list-disputes-pending-settlement-python.mdx';
import NetListDisputesPendingSettlement from '/snippets/code/net-list-disputes-pending-settlement.mdx';

This call lists all the disputes for which you need to make a settlement transfer.  

### Responses  

<AccordionGroup><Accordion title="200">  

<Array_DisputesResponse /></Accordion></AccordionGroup>  

<ResponseExample>  

```json 200
[
    {
        "InitialTransactionId": "158596153",
        "InitialTransactionType": "PAYIN",
        "InitialTransactionNature": "REGULAR",
        "DisputeType": "CONTESTABLE",
        "ContestDeadlineDate": 1673049599,
        "DisputedFunds": {
            "Currency": "EUR",
            "Amount": 1200
        },
        "ContestedFunds": {
            "Currency": "EUR",
            "Amount": 1200
        },
        "Status": "CLOSED",
        "StatusMessage": null,
        "DisputeReason": {
            "DisputeReasonMessage": "This is a test dispute",
            "DisputeReasonType": "UNKNOWN"
        },
        "ResultCode": "LOST",
        "ResultMessage": null,
        "Id": "159102965",
        "Tag": null,
        "CreationDate": 1672411848,
        "ClosedDate": null,
        "RepudiationId": "159102966"
    }
]  

 ```  

</ResponseExample>  

<RequestExample>  
    <NodeListDisputesPendingSettlement />
    <JavaListDisputesPendingSettlement />
    <ListDisputesPendingSettlementPython />
    <NetListDisputesPendingSettlement />
</RequestExample>