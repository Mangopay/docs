---
title: 'Create a BLIK PayIn (without code)'  
api: 'POST /v2.01/{ClientId}/payins/payment-methods/blik'  
---

import StatementDescriptorResponse from '/snippets/p-response/StatementDescriptorResponse.mdx';
import RedirectURLResponse from '/snippets/p-response/RedirectURLResponse.mdx';
import ReturnURLResponse from '/snippets/p-response/ReturnURLResponse.mdx';
import ExecutionTypeResponse from '/snippets/p-response/ExecutionTypeResponse.mdx';
import PaymentType_Response_Blik from '/snippets/p-response/PaymentType_Response_Blik.mdx';
import CreditedWalletIdResponse from '/snippets/p-response/CreditedWalletIdResponse.mdx';
import NatureResponse from '/snippets/p-response/NatureResponse.mdx';
import TypeResponse from '/snippets/p-response/TypeResponse.mdx';
import ExecutionDateResponse from '/snippets/p-response/ExecutionDateResponse.mdx';
import ResultMessageResponse from '/snippets/p-response/ResultMessageResponse.mdx';
import ResultCodeResponse from '/snippets/p-response/ResultCodeResponse.mdx';
import Status2Response from '/snippets/p-response/Status2Response.mdx';
import Fees3Response from '/snippets/p-response/Fees3Response.mdx';
import CreditedFunds4Response from '/snippets/p-response/CreditedFunds4Response.mdx';
import DebitedFunds4Response from '/snippets/p-response/DebitedFunds4Response.mdx';
import AuthorIdResponse from '/snippets/p-response/AuthorIdResponse.mdx';
import CreationDateResponse from '/snippets/p-response/CreationDateResponse.mdx';
import TagResponse from '/snippets/p-response/TagResponse.mdx';
import IdResponse from '/snippets/p-response/IdResponse.mdx';
import Code1Response from '/snippets/p-response/Code1Response.mdx';
import BrowserInfo2Response from '/snippets/p-response/BrowserInfo2Response.mdx';
import IpAddressResponse from '/snippets/p-response/IpAddressResponse.mdx';

import StatementDescriptor from '/snippets/p-body/StatementDescriptor.mdx';
import ReturnURL from '/snippets/p-body/ReturnURL.mdx';
import Fees3 from '/snippets/p-body/Fees3.mdx';
import DebitedFunds4 from '/snippets/p-body/DebitedFunds4.mdx';
import CreditedWalletId from '/snippets/p-body/CreditedWalletId.mdx';
import Tag from '/snippets/p-body/Tag.mdx';
import AuthorId from '/snippets/p-body/AuthorId.mdx';
import IpAddress from '/snippets/p-body/IpAddress.mdx';
import Code from '/snippets/p-body/Code.mdx';
import BrowserInfo1 from '/snippets/p-body/BrowserInfo1.mdx';

import RestCreateBlikPayinWithoutCode from '/snippets/code/rest-create-blik-payin-without-code.mdx';
import JavaCreateBlikPayinWithoutCode from '/snippets/code/java-create-blik-payin-without-code.mdx';
import PythonCreateBlikPayinWithoutCode from '/snippets/code/python-create-blik-payin-without-code.mdx';
import NetCreateBlikPayinWithoutCode from '/snippets/code/net-create-blik-payin-without-code.mdx';

### Body parameters  

<AuthorId />  

<Tag />  

<CreditedWalletId />  

<DebitedFunds4 />  

<Fees3 />  

<ReturnURL />  

<StatementDescriptor />  

### Responses  

<AccordionGroup><Accordion title="200 - Created">  

<IdResponse />  

<TagResponse />  

<CreationDateResponse />  

<AuthorIdResponse />  

<DebitedFunds4Response />  

<CreditedFunds4Response />  

<Fees3Response />  

<Status2Response />  

<ResultCodeResponse />  

<ResultMessageResponse />  

<ExecutionDateResponse />  

<TypeResponse />  

<NatureResponse />  

<CreditedWalletIdResponse />  

<PaymentType_Response_Blik />  

<ExecutionTypeResponse />  

<ReturnURLResponse />  

<RedirectURLResponse />  

<StatementDescriptorResponse />

<Code1Response />

<BrowserInfo2Response />

<IpAddressResponse /></Accordion></AccordionGroup>  

<ResponseExample>  

```json 200 - Created  
{
    "Id": "wt_dbb46bf7-71e1-463c-bbd5-8217f3c4b474",
    "Tag": "Created using the Mangopay API Postman collection",
    "CreationDate": 1715610590,
    "AuthorId": "user_m_01HWSRS7HK6S0XKKD4CJM3PQ04",
    "DebitedFunds": {
        "Currency": "PLN",
        "Amount": 100
    },
    "CreditedFunds": {
        "Currency": "PLN",
        "Amount": 100
    },
    "Fees": {
        "Currency": "PLN",
        "Amount": 0
    },
    "Status": "CREATED",
    "ResultCode": null,
    "ResultMessage": null,
    "ExecutionDate": null,
    "Type": "PAYIN",
    "Nature": "REGULAR",
    "CreditedWalletId": "wlt_m_01HWSSQV8W6550FJ5NTJQE1DE1",
    "CreditedUserId": "user_m_01HWSRS7HK6S0XKKD4CJM3PQ04",
    "PaymentType": "BLIK",
    "ExecutionType": "WEB",
    "ReturnURL": "https://www.mangopay.com/docs/please-ignore?transactionId=wt_dbb46bf7-71e1-463c-bbd5-8217f3c4b474",
    "RedirectURL": "https://r3.girogate.de/ti/dumbdummy?tx=2246577013&rs=D4a2bjG5roOlXDEvsBOdb2NoCNtPxxKZ&cs=f3452531239c64143ec6a9ccd382553ec71a987cdcdc16a4f2046bc5e3b8d373",
    "StatementDescriptor": "May2024",
    "Code": null,
    "BrowserInfo": null,
    "IpAddress": null
}
 ```  

</ResponseExample>  

<RequestExample>
  
    <RestCreateBlikPayinWithoutCode />
    <JavaCreateBlikPayinWithoutCode />
    <PythonCreateBlikPayinWithoutCode />
    <NetCreateBlikPayinWithoutCode />
    
</RequestExample>