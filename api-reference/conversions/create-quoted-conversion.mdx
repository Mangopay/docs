---
title: 'Create a Quoted Conversion (Guaranteed FX)'  
api: 'POST /v2.01/{ClientId}/conversions/quoted-conversion'  
---

import TagResponse from '/snippets/response-parameters/TagResponse.mdx';
import ConversionRateResponseResponse from '/snippets/response-parameters/ConversionRateResponseResponse.mdx';
import ExecutionDateResponse from '/snippets/response-parameters/ExecutionDateResponse.mdx';
import ResultMessageResponse from '/snippets/response-parameters/ResultMessageResponse.mdx';
import ResultCodeResponse from '/snippets/response-parameters/ResultCodeResponse.mdx';
import CreditedFundsResponse from '/snippets/response-parameters/CreditedFundsResponse.mdx';
import DebitedFundsResponse from '/snippets/response-parameters/DebitedFundsResponse.mdx';
import CreditedWalletId3Response from '/snippets/response-parameters/CreditedWalletId3Response.mdx';
import DebitedWalletId2Response from '/snippets/response-parameters/DebitedWalletId2Response.mdx';
import AuthorId4Response from '/snippets/response-parameters/AuthorId4Response.mdx';
import Status2Response from '/snippets/response-parameters/Status2Response.mdx';
import CreationDateResponse from '/snippets/response-parameters/CreationDateResponse.mdx';
import NatureResponse from '/snippets/response-parameters/NatureResponse.mdx';
import Type9Response from '/snippets/response-parameters/Type9Response.mdx';
import QuoteIdResponse from '/snippets/response-parameters/QuoteIdResponse.mdx';
import IdResponse from '/snippets/response-parameters/IdResponse.mdx';
import Tag from '/snippets/body-parameters/Tag.mdx';
import CreditedWalletId1 from '/snippets/body-parameters/CreditedWalletId1.mdx';
import DebitedWalletId1 from '/snippets/body-parameters/DebitedWalletId1.mdx';
import AuthorId4 from '/snippets/body-parameters/AuthorId4.mdx';
import QuoteId from '/snippets/body-parameters/QuoteId.mdx';
  

This call triggers a conversion, at the rate guaranteed by its quote, of the debited funds to the credited wallet.  

Each quoted conversion requires a dedicated Quote object, linked in the `QuoteId`.   

Fees can’t be taken on a quoted conversion.  

### Body parameters  

<QuoteId />  

<AuthorId4 />  

<DebitedWalletId1 />  

<CreditedWalletId1 />  

<Tag />  

### Responses  

<AccordionGroup><Accordion title="200 Succeeded">  

<IdResponse />  

<QuoteIdResponse />  

<Type9Response />  

<NatureResponse />  

<CreationDateResponse />  

<Status2Response />  

<AuthorId4Response />  

<DebitedWalletId2Response />  

<CreditedWalletId3Response />  

<DebitedFundsResponse />  

<CreditedFundsResponse />  

<ResultCodeResponse />  

<ResultMessageResponse />  

<ExecutionDateResponse />  

<ConversionRateResponseResponse />  

<TagResponse /></Accordion><Accordion title="400 Quote required for Quoted Conversion"></Accordion><Accordion title="400 Quote expired"></Accordion><Accordion title="400 Quote already used"></Accordion><Accordion title="400 Credited currency doesn’t match quote"></Accordion><Accordion title="400 Debited currency doesn’t match quote"></Accordion><Accordion title="400 Author is not debited wallet owner"></Accordion><Accordion title="400 Author is not credited wallet owner "></Accordion></AccordionGroup>  

<ResponseExample>  

```json 200 Succeeded  

{
    "Id": "cvr_01HP1H6P56SSSTBZK1K9THFP79",
    "QuoteId": "cvrquote_01HP1H6EK4H39SCQ8WJ349WMBB",
    "Type": "CONVERSION",
    "Nature": "REGULAR",
    "CreationDate": 1707301099,
    "Status": "SUCCEEDED",
    "AuthorId": "204071581",
    "DebitedWalletId": "204079338",
    "CreditedWalletId": "209408867",
    "DebitedFunds": {
        "Currency": "GBP",
        "Amount": 1000
    },
    "CreditedFunds": {
        "Currency": "USD",
        "Amount": 1263
    },
    "ResultCode": "000000",
    "ResultMessage": "Success",
    "ExecutionDate": 1707301100,
    "ConversionRateResponse": {
        "ClientRate": 1.2504,
        "MarketRate": 1.263
    },
    "Tag": "Created using the Mangopay API Postman collection"
}  

 ```  

```json 400 Quote required for Quoted Conversion  

{
    "Message": "One or several required parameters are missing or incorrect. An incorrect resource ID also raises this kind of error.",
    "Type": "param_error",
    "Id": "ffaf5ad6-d9f1-47bf-832c-ea5da626037e",
    "Date": 1706192320,
    "errors": {
        "QuoteId": "Quote not found"
    }
}  

 ```  

```json 400 Quote expired  

{
    "Message": "One or several required parameters are missing or incorrect. An incorrect resource ID also raises this kind of error.",
    "Type": "param_error",
    "Id": "31dc86e1-783e-4e34-94d0-4e1891fd74bb",
    "Date": 1707301282,
    "errors": {
        "QuoteId": "The quote is expired"
    }
}  

 ```  

```json 400 Quote already used  

{
    "Message": "One or several required parameters are missing or incorrect. An incorrect resource ID also raises this kind of error.",
    "Type": "param_error",
    "Id": "2b989396-3dc7-4fcf-8a7e-2c6463262633",
    "Date": 1707301113,
    "errors": {
        "QuoteId": "The quote is already consumed"
    }
}  

 ```  

```json 400 Credited currency doesn’t match quote  

{
    "Message": "Credited currency incompatibility.",
    "Type": "currency_incompatibility",
    "Id": "47ade3b3-0c5d-4c32-ae11-1cfd23720a11",
    "Date": 1707314958,
    "errors": null
}  

 ```  

```json 400 Debited currency doesn’t match quote  

{
    "Message": "Debited currency incompatibility.",
    "Type": "currency_incompatibility",
    "Id": "f83e073e-287d-4781-bbba-87256dad4aa0",
    "Date": 1707315285,
    "errors": null
}  

 ```  

```json 400 Author is not debited wallet owner  

{
    "Message": "Author 204071581 is not debited wallet 214818911 owner.",
    "Type": "author_is_not_debited_wallet_owner",
    "Id": "c16cb140-0f02-4c5f-a705-9cfdb11fb499",
    "Date": 1708382645.0,
    "errors": null
}  

 ```  

```json 400 Author is not credited wallet owner   

{
    "Message": "Author 204071581 is not credited wallet 214818911 owner.",
    "Type": "author_is_not_credited_wallet_owner",
    "Id": "5586bf7e-8e33-4e7a-a140-107f7a9e3a26",
    "Date": 1708382704.0,
    "errors": null
}  

 ```  

</ResponseExample>  

<RequestExample>  
  
```json REST  

{
    "QuoteId": "cvrquote_01HP1H6EK4H39SCQ8WJ349WMBB",
    "AuthorId": "204071581",
    "DebitedWalletId": "204079338",
    "CreditedWalletId": "209408867",
    "Tag": "Created using the Mangopay API Postman collection"
}  
```  

  
</RequestExample>