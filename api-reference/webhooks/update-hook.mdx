---
title: 'Update a Hook'  
api: 'PUT /v.2.01/{ClientId}/hooks/{HookId}'  
---

import CreationDateResponse from '/snippets/p-response/CreationDateResponse.mdx';
import Tag2Response from '/snippets/p-response/Tag2Response.mdx';
import IdResponse from '/snippets/p-response/IdResponse.mdx';
import EventTypeResponse from '/snippets/p-response/EventTypeResponse.mdx';
import Validity1Response from '/snippets/p-response/Validity1Response.mdx';
import Status7Response from '/snippets/p-response/Status7Response.mdx';
import UrlResponse from '/snippets/p-response/UrlResponse.mdx';
import Tag3 from '/snippets/p-body/Tag3.mdx';
import Status1 from '/snippets/p-body/Status1.mdx';
import Url from '/snippets/p-body/Url.mdx';
import HookId from '/snippets/p-path/HookId.mdx';

import RestUpdateHook from '/snippets/code/rest-update-hook.mdx';
import PhpUpdateHook from '/snippets/code/php-update-hook.mdx';
import NodeUpdateHook from '/snippets/code/node-update-hook.mdx';
import RubyHookUpdate from '/snippets/code/ruby-hook-update.mdx';
import JavaUpdateHook from '/snippets/code/java-update-hook.mdx';
import UpdateHookPython from '/snippets/code/update-hook-python.mdx';
import NetUpdateHook from '/snippets/code/net-update-hook.mdx';

**Scope:** This endpoint requires the `hooks_write` scope.

### Path parameters  

<HookId />  

### Body parameters  

<Url />  

<Status1 />  

<Tag3 />  

### Responses  

<AccordionGroup><Accordion title="200">  

<UrlResponse />  

<Status7Response />  

<Validity1Response />  

<EventTypeResponse />  

<IdResponse />  

<Tag2Response />  

<CreationDateResponse /></Accordion></AccordionGroup>  

<ResponseExample>  

```json 200  

{
    "Url": "https://mangopay.com/docs/please-ignore",
    "Status": "DISABLED",
    "Validity": "VALID",
    "EventType": "UBO_DECLARATION_VALIDATION_ASKED",
    "Id": "144246103",
    "Tag": "Custom meta",
    "CreationDate": 1655991027
}  
 ```  

</ResponseExample>  

<RequestExample>  
    <RestUpdateHook />
    <PhpUpdateHook />
    <NodeUpdateHook />
    <RubyHookUpdate />
    <JavaUpdateHook />
    <UpdateHookPython />
    <NetUpdateHook />
</RequestExample>