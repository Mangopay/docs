---
title: 'Create a Bank Wire PayIn'  
api: 'POST /v2.01/{ClientId}/payins/bankwire/direct/'  
---

import TransactionDetailsResponse from '/snippets/p-response/TransactionDetailsResponse.mdx';
import BankAccountResponse from '/snippets/p-response/BankAccountResponse.mdx';
import WireReferenceResponse from '/snippets/p-response/WireReferenceResponse.mdx';
import DeclaredFeesResponse from '/snippets/p-response/DeclaredFeesResponse.mdx';
import DeclaredDebitedFundsResponse from '/snippets/p-response/DeclaredDebitedFundsResponse.mdx';
import ExecutionTypeResponse from '/snippets/p-response/ExecutionTypeResponse.mdx';
import PaymentType_Response_BankWire from '/snippets/p-response/PaymentType_Response_BankWire.mdx';
import DebitedWalletId1Response from '/snippets/p-response/DebitedWalletId1Response.mdx';
import CreditedWalletIdResponse from '/snippets/p-response/CreditedWalletIdResponse.mdx';
import NatureResponse from '/snippets/p-response/NatureResponse.mdx';
import TypeResponse from '/snippets/p-response/TypeResponse.mdx';
import ExecutionDateResponse from '/snippets/p-response/ExecutionDateResponse.mdx';
import Status2Response from '/snippets/p-response/Status2Response.mdx';
import Fees7Response from '/snippets/p-response/Fees7Response.mdx';
import CreditedFunds10Response from '/snippets/p-response/CreditedFunds10Response.mdx';
import DebitedFunds15Response from '/snippets/p-response/DebitedFunds15Response.mdx';
import CreditedUserIdResponse from '/snippets/p-response/CreditedUserIdResponse.mdx';
import AuthorIdResponse from '/snippets/p-response/AuthorIdResponse.mdx';
import ResultMessageResponse from '/snippets/p-response/ResultMessageResponse.mdx';
import ResultCodeResponse from '/snippets/p-response/ResultCodeResponse.mdx';
import CreationDateResponse from '/snippets/p-response/CreationDateResponse.mdx';
import Tag2Response from '/snippets/p-response/Tag2Response.mdx';
import IdResponse from '/snippets/p-response/IdResponse.mdx';

import ProfilingAttemptReference from '/snippets/p-body/ProfilingAttemptReference.mdx';
import DeclaredFees from '/snippets/p-body/DeclaredFees.mdx';
import DeclaredDebitedFunds from '/snippets/p-body/DeclaredDebitedFunds.mdx';
import CreditedWalletId from '/snippets/p-body/CreditedWalletId.mdx';
import CreditedUserId from '/snippets/p-body/CreditedUserId.mdx';
import AuthorId from '/snippets/p-body/AuthorId.mdx';
import Tag from '/snippets/p-body/Tag.mdx';
  
import RestCreateBankWirePayin from '/snippets/code/rest-create-bank-wire-payin.mdx';
import PhpCreateBankWirePayin from '/snippets/code/php-create-bank-wire-payin.mdx';
import NodeCreateBankWirePayin from '/snippets/code/node-create-bank-wire-payin.mdx';
import RubyCreateBankWirePayin from '/snippets/code/ruby-create-bank-wire-payin.mdx';
import JavaCreateBankWirePayin from '/snippets/code/java-create-bank-wire-payin.mdx';
import PythonCreateBankWirePayin from '/snippets/code/python-create-bank-wire-payin.mdx';
import NetCreateBankWirePayin from '/snippets/code/net-create-bank-wire-payin.mdx';

This call declares the payment to be made by the end user and returns the `BankAccount` details and `WireReference` they must use.

<Warning>
**Caution – Retrieve the returned bank details dynamically**

In addition to the `WireReference`, ensure you implement the `BankAccount` object parameter dynamically so the `IBAN`, `BIC`, and other response values appear as returned in for each pay-in.

Mangopay may change the underlying bank account without prior notice and mismatched data my lead to delays. See the <a href="/guides/payment-methods/banking/bank-wire">bank wire pay-in</a> guide for more details.
</Warning>

### Body parameters  

<Tag />  

<AuthorId />  

<CreditedUserId />  

<CreditedWalletId />  

<DeclaredDebitedFunds />  

<DeclaredFees />  

<ProfilingAttemptReference />  

### Responses  

<AccordionGroup>

<Accordion title="200" defaultOpen>  

<IdResponse />  

<Tag2Response />  

<CreationDateResponse />  

<ResultCodeResponse />  

<ResultMessageResponse />  

<AuthorIdResponse />  

<CreditedUserIdResponse />  

<DebitedFunds15Response /> 

<CreditedFunds10Response />  

<Fees7Response />  

<Status2Response />  

<ExecutionDateResponse />  

<TypeResponse />  

<NatureResponse />  

<CreditedWalletIdResponse />  

<DebitedWalletId1Response />  

<PaymentType_Response_BankWire />  

<ExecutionTypeResponse />  

<DeclaredDebitedFundsResponse />  

<DeclaredFeesResponse />  

<WireReferenceResponse />  

<BankAccountResponse />

<TransactionDetailsResponse />

</Accordion>

</AccordionGroup>  

<ResponseExample>  

```json 200 - CREATED
{
    "Id": "payin_m_01JFAJSSR3VW3SHX2X74C8GW82",
    "Tag": "Created using Mangopay API Postman Collection",
    "CreationDate": 1734448310,
    "ResultCode": null,
    "ResultMessage": null,
    "AuthorId": "user_m_01JCQYJNHFPENP1SKTCBYER0F8",
    "CreditedUserId": "user_m_01JCQYJNHFPENP1SKTCBYER0F8",
    "DebitedFunds": {
        "Currency": "XXX",
        "Amount": 0
    },
    "CreditedFunds": {
        "Currency": "XXX",
        "Amount": 0
    },
    "Fees": {
        "Currency": "XXX",
        "Amount": 0
    },
    "Status": "CREATED",
    "ExecutionDate": null,
    "Type": "PAYIN",
    "Nature": "REGULAR",
    "CreditedWalletId": "wlt_m_01JFAJS7838A32W7KPJ7NNGJ1D",
    "DebitedWalletId": null,
    "PaymentType": "BANK_WIRE",
    "ExecutionType": "DIRECT",
    "DeclaredDebitedFunds": {
        "Currency": "EUR",
        "Amount": 62789
    },
    "DeclaredFees": {
        "Currency": "EUR",
        "Amount": 7826
    },
    "WireReference": "MGgq1y8jjb",
    "BankAccount": {
        "OwnerAddress": {
            "AddressLine1": "2 Avenue Amélie",
            "AddressLine2": null,
            "City": "Luxembourg",
            "Region": null,
            "PostalCode": "L-1125",
            "Country": "LU"
        },
        "Type": "IBAN",
        "OwnerName": "MANGOPAY SA",
        "IBAN": "FR7630056009271234567890182",
        "BIC": "CCFRFRPPXXX"
    },
    "TransactionDetails": []
}
```

</ResponseExample>  

<RequestExample>  

<RestCreateBankWirePayin />
<PhpCreateBankWirePayin />
<NodeCreateBankWirePayin />
<RubyCreateBankWirePayin />
<JavaCreateBankWirePayin />
<PythonCreateBankWirePayin />
<NetCreateBankWirePayin />

</RequestExample>