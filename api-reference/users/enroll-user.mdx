...
This endpoint allows you to enroll users whose `UserCategory` is `OWNER` by redirecting them on the `PendingUserAction.RedirectUrl` response value to complete SCA enrollment.

You can use this endpoint for Owners created without SCA enrollment or if a previous attempt at enrollment was not successful.
