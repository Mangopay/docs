...
This endpoint allows you to transition a user whose `UserCategory` is `PAYER` into an `OWNER` by providing the required information and obtaining an SCA session link in the `PendingUserAction.RedirectUrl` response field.

If the user's `UserCategory` is already `OWNER`, use the [PUT Activate a User](/api-reference/users/activate-user) endpoint to obtain an SCA session link.
...
If the user's `UserCategory` is `OWNER`, use the [PUT Activate a User](/api-reference/users/activate-user) endpoint to obtain an SCA session link.
</Accordion>
</AccordionGroup>
