---
title: "The IDV Session object"
description: "Container for Mangopay's hosted KYC/KYB session"
---

import IdResponse from '/snippets/p-response/IdResponse.mdx';
import Tag2Response from '/snippets/p-response/Tag2Response.mdx';
import HostedUrlResponse from '/snippets/p-response/HostedUrlResponse.mdx';
import ReturnURL3Response from '/snippets/p-response/ReturnURL3Response.mdx';
import Status13Response from '/snippets/p-response/Status13Response.mdx';
import UserId2Response from '/snippets/p-response/UserId2Response.mdx';
import CreationDateResponse from '/snippets/p-response/CreationDateResponse.mdx';
import LastUpdate1Response from '/snippets/p-response/LastUpdate1Response.mdx';
import ChecksResponse from '/snippets/p-response/ChecksResponse.mdx';

### Description 

Mangopay relies on the IDV Session object to provide its hosted KYC/KYB verification solution. The object contains the unique session URL for the user's session where they can provide all the necessary data and documents for verification. [Read guide](/guides/users/verification/hosted) **&rarr;**

### Attributes

<IdResponse/>
<Tag2Response/>
<HostedUrlResponse/>
<ReturnURL3Response/>
<Status13Response/>
<UserId2Response/>
<CreationDateResponse/>
<LastUpdate1Response/>
<ChecksResponse/>
