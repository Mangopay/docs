...
Part of the original Intent `Amount` is captured from one or more line items. The `ExternalData` references are/or date are different from the Intent. The Capture `Amount` must equal the sum of the `Amount` values captured for all line items.
