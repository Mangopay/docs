...
Part of the original Intent `Amount` is canceled from one or more line items. The `ExternalData` references and date must be provided as cancellation was necessarily a different action taken via the third-party PSP. The cancellation `Amount` must equal the sum of its line items.
